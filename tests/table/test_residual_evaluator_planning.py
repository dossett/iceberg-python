...
from pyiceberg.table import DataScan, Table, TableProperties
...
def test_data_scan_reuses_residuals_by_spec_and_partition(table_v2: Table) -> None:
    entries = [
    ...
    scan = _scan(table_v2, EqualTo("x", 1), entries, evaluators, _identity_spec(0, 1), _identity_spec(1, 1))

    tasks = list(scan._plan_files_local())

    assert scan.evaluator_builds == [0, 1]
...
def test_data_scan_ignores_unreferenced_partition_fields(table_v2: Table) -> None:
    entries = [
    ...
    scan = _scan(table_v2, EqualTo("x", 1), entries, {0: evaluator}, _identity_spec(0, 1, 2))
...
def test_data_scan_includes_referenced_partition_fields(table_v2: Table) -> None:
    entries = [
        _manifest_entry(0, spec_id=0, partition=(1, 10)),
        _manifest_entry(1, spec_id=0, partition=(1, 20)),
    ]
...
def test_data_scan_includes_all_partition_transforms_for_referenced_source(table_v2: Table) -> None:
    spec = PartitionSpec(
    ...
    evaluator = _CountingResidualEvaluator(0)
    scan = _scan(table_v2, EqualTo("x", 1), entries, {0: evaluator}, spec)
...
def test_data_scan_bounds_residual_cache(table_v2: Table) -> None:
    entries = [
        _manifest_entry(0, spec_id=0, partition=(1,)),
